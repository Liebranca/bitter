...
MEM* CRK_VRTS;                  // a vertex buffer
                                // (keep it small)

// ---   *   ---   *   ---
// verts read on each run

#define CRK_READCOUNT           \
    xELEMS(ZJC_DAFPAGE, CRKVRT) \
    /ZJC_RAWVRT_ELEMS
...
BIN* MKCRK (
  ...
  BINOPEN (crk, path, mode, KVR_FTYPE_CRK, page,
           evilstate);
...
// set fname and create/open files
  ...
  if (CRK_FROM == (BIN *) ERROR) {
    return ERROR;
  };

  // ensure mezz is wiped when we *write* to it
  int mezz_rmode =(mode == CRK_ENCODE)
    ? KVR_FMODE_WB
    : KVR_FMODE_RBP;

  RPSTR (&m, "MEZZ.crk", strlen (path) - 11);
  BINOPEN (CRK_TO, path, mezz_rmode, KVR_FTYPE_CRK, 0,
           evilstate);
  ...
  if (mode == CRK_DECODE) {     // castling on invert
                                // op
    BIN* tmp = CRK_FROM;
...
// make recycle vertex buffer
  ...
  uint page = xBYTES (CRK_READCOUNT, float
  ) * ZJC_RAWVRT_ELEMS;
...
int DLCRKENG (
  ...
  if (mode == CRK_DECODE) {     // de-castling on
                                // invert op
    BIN* tmp = CRK_FROM;
...
// ---   *   ---   *   ---
  ...
  *size_i += dim * sizeof (CRKVRT);
  ...
  // write indices
  wb =
    BIN2BIN (CRK_VRTDUMP, CRK_FROM,
             (crkh->idex_count * 3) *
             sizeof (crkh->idex_count));
...
// ---   *   ---   *   ---
  ...
  while (remain) {              // read one vert-wide
                                // page at a time

    uint readsize = (remain < page) ? remain : page;

    // load buffer from dump
    BINREAD (CRK_VRTDUMP, wb, float,
             readsize,
             verts
    );

    remain -= readsize;         // discount read from
                                // pending

    for (uint x = 0, y = 0; x < (readsize);
         x += ZJC_RAWVRT_ELEMS, y++

    ) {CRKENC_F (verts + x, buff + y);};
...
int ZPCRK (
  ...
  uint offs_d =
    (sizeof (uint) * 3) + (count * sizeof (CRKH));
  ...
  // zip the entire file
  DEFLBIN (CRK_FROM, CRK_TO, size_i, &size_d, offs_i,
           offs_d);
